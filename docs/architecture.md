...
| 备份即一个数据库快照           | 全部用户数据（含照片、参考文件、助手图片附件）都在同一个 SQLite 文件里，所以导出=一致性快照 + 元信息 JSON，恢复=替换该文件；导出物不含 API Key，且清空密钥必须配合 `VACUUM` 重建文件，只 `UPDATE` 会留下空闲页残留 |
| 恢复先备份再替换、失败自动回滚 | 覆盖用户全部数据的操作，必须可撤销：替换前自动备份当前库，替换后升级失败则换回原数据；上传与恢复分两步并用一次性 token 关联，让用户在覆盖前看到备份内容 |
