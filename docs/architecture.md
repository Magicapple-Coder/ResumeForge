...
投递台按关键词从招聘网站搜岗位；**官网采集**从用户指定的公司招聘官网读岗位。它要回答的问题不是"抓到了多少条"，而是**"这次抓全了没有"**——而"没报错"与"抓全了"是两件不同的事：一次被 429 截断的采集同样是"没报错"，只是少了一半岗位。

```mermaid
sequenceDiagram
    participant U as 用户
    participant F as 前端官网采集页
    participant A as /api/official 路由
    participant S as services/sites/official 编排
    participant G as RobotsAwareHttp 闸门
    participant P as 公司招聘官网 / 招聘系统接口
    participant DB as SQLite

    U->>F: 添加公司（公司名 + 官网/招聘页地址）
    F->>A: POST /api/official/sites
    A->>S: probe_and_store
    S->>G: 查该主机的 robots.txt
    G-->>S: 允许 / 不允许 / 读不到（读不到视为不允许）
    S->>G: 取首页 + 试各系统的候选端点
    G->>P: 逐步 SSRF 校验后取回
    P-->>G: 响应
    G-->>S: FetchResult（带阻断分类）
    S->>DB: 写 official_site（识别结果 + robots 结论 + 证据）
    U->>F: 点「采集」
    F->>A: POST /api/official/sites/{id}/collect
    A->>S: collect_site
    loop 翻页（上限 30 页）
        S->>G: fetch_page(cursor)
        G->>P: 取一页
        S->>S: 去重 → 落备选岗位（candidate_job）
    end
    S->>S: reconcile：终止判定 + 集合/总量/列表对账
    S->>DB: 写 official_collect_run（账目 + 三态结论 + 逐层依据）
    S-->>F: 报告（结论 + 逐层依据）
```

- **读路径不复用投递的 `SiteAdapter`**：那个契约要求子类实现 `open_apply` / `fill_and_submit`，而采集是纯读且大多走 HTTP。塞进同一个抽象的结果是每个采集适配器都要实现一个永远用不到的投递方法。两者**共享数据结构**（`RiskProfile` / `CollectQuery`）但各自定义行为接口，注册表也并列而非合并。
- **公司发现接口保留兼容但不再是产品入口**：当前官网采集只从用户自己添加的公司官网 / 招聘页开始，前端不再暴露岗位需求搜公司、搜索历史或候选公司清单；旧路由与历史表暂留，避免旧客户端和历史数据升级时断裂。
- **三级降级：接口 → 页面内嵌结构化数据 → 通用抽取**。公司招聘页背后几乎总是某套成熟的招聘系统，其中相当一部分提供**无鉴权公开 JSON 接口**且返回体里带总数——接口是契约、网页是易碎品，而那个总数正是对账能给出硬结论的前提。探测**一次、复用多次**：识别结果连同端点落进 `official_site`，之后每次采集直接走契约。**命中判据是"端点存在且返回 ≥1 条岗位"**：有的系统在公司不存在时也返回 200 且总数为 0，只看"端点存在"就会把"这家公司不用这套系统"读成"这家公司在用、只是没在招"，而后者会让对账报出"0 条，已确认为全量"。唯一例外是用户自己给的招聘页地址——那是他亲眼看到的页面，本身就是证据，此时 0 岗位是真实答案。
- **对账只有三态，不出百分比分数**（与「投递进度不显示百分比」「匹配度分析不显示百分比」同一取向）：**已确认为全量**（存在总数契约且账目相符，或可枚举全集被覆盖）/ **已确认不全**（差额确切）/ **无法确认**（没有依据、被阻断、或信号矛盾）。百分比会给出虚假的精确感，而三态各自都有可核对的依据，报告页按层展开（`services/sites/official/reconcile.py`）。
- **翻页终止是双信号判定**，这是整个功能最容易做错的一处。内容侧证据（翻页控件消失、游标不推进）→ 确认到底；传输侧证据（429 / 403 / 验证码 / 登录墙 / **软封禁** / 超时）→ "不知道那边还有什么"；两者矛盾 → 降级为无法确认。**达到页数上限与用户手动停止都不算"抓到底了"**，且两者分开记录——一个是工具的边界，一个是用户的决定。第 2 页起出现 404 也判为矛盾：既可能是"没有下一页"，也可能是"接口改了"，而后者意味着从那一页起全丢了，两种含义的后果差别极大，所以不猜。
- **对总数比的是"去重后的取回条数"，不是入库条数**。已在岗位广场或暂存区的岗位会被跳过，但它们**确实被这一次采集抓到了**；用入库条数对总数，会把"命中大量重复"误报成"漏抓了大量岗位"——一个让用户白跑一趟的假警报。
- **阻断分类是一等公民**（`models/official.py`），因为它直接决定对账结论。取回层（`services/sites/official/http.py`）把状态码、超时、连接失败、**截断**都收敛成分类；`blocking.py` 再从正文里识别验证码页 / 登录墙 / 空壳页——**状态码 200 不等于拿到了内容**，拦截页普遍以 200 返回，不识别它就会把"被拦住"读成"这里没有岗位"。这个模块的判据取向是**宁可多判阻断**（假阳性只是让结论保守地降级为"无法确认"，假阴性则会让报告自信地宣称已确认为全量），但 marker 只用明确的整句或协议特征，绝不用「登录」这种单词——导航栏里那个「登录」链接会让每一次采集都变成"需要登录"。
- **robots.txt 按主机生效，并且装在传输层**。一次采集至少碰到两个主机：用户给的招聘页域名与适配器实际调用的接口域名。只查前者，我们就遵守了用户看到的那个站点的规则，却对真正被打的接口域名一无所知。`RobotsAwareHttp` 把判定做成传输层装饰器——适配器拿到的是它，**不存在"某个适配器忘了查"这种可能**，与 SSRF 防护放在同一处的理由完全相同（防护在 `http.py` 的唯一实现里，适配器碰不到 `httpx`）。包装必须幂等：嵌套一层会导致取 `robots.txt` 自身时又去查它自己的 robots。判定取向是**读不到规则时保守视为不允许**——"读不到就当作没有规则"会把站点故障期变成被加压的窗口；而 404（文件确实不存在）按协议语义视为允许。
- **SSRF 防护复用搜索层的 `is_public_http_url`，但跳转自己做**：`page_reader` 用 `follow_redirects=True` 交给 httpx 内部处理，**跳转目标不会被重新校验**——一个公网地址就能把本机应用变成打内网的跳板。`http.py` 因此手动跟跳转，**每一跳都重新过一遍防护**。两者都只解析一次 DNS，本机单用户工具不追求抵抗 DNS rebinding（不假装它更强）。
- **不伪装身份**：UA 如实标识为 `ResumeForge/<版本> (+本机求职工具)`。需要浏览器的场景（JS 渲染、登录态）走 CDP 用**真浏览器**，那是真的而不是假装——伪装 UA 的唯一作用是让站点无法识别访问者身份，而那恰好也是站点最不欢迎的行为，代价是我们在被误伤时连一个可追溯的联系方式都没有。
- **模型只用于结构化抽取，导航永远由确定性代码执行**——因为"模型说抓完了"不能作为完整度证据。首发适配器（`feeds/greenhouse.py`）甚至完全不用模型：它的公开接口直接返回结构化岗位。该适配器有一条实测约束写在里面：`content=true` 的响应体可以到几十 MB 量级，默认的字节上限会把它截断，而截断在上层会被判成"不可信"——于是每次采集都变成"无法确认"。**"多大算过大"只有适配器知道**，所以上限由适配器声明。
- **采集结果先进备选岗位**（与投递台的采集一致），**不接入自动投递**。各家公司的投递表单千差万别，而投递台有一条硬约束（只接纳已注册适配器的站点）；让陌生表单进入投递路径，填错的代价是用户的真实误投。官网采集来的岗位在岗位详情里会如实地显示"不能用投递台自动投递"。
- **编排层是"地址队列"而不是"游标翻页"**。两种爬取形状共用它：接口型站点一个端点返回全部岗位、翻页靠游标；列表页型站点则是"列表页 → 一堆详情页"。泛化的代价只是 `FeedPage` 多一个 `next_targets`，换来的是**去重、落库、限速、对账四件事仍然只有一份实现**——给通用路径另写一套编排，等于让这四件事各有两份，而它们必然漂移。队列走空是唯一不需要额外信号即可成立的终止条件；另有深度上限与已访问判重，防止自身链接成环的站点把采集带进无限递归。
- **通用路径（无专用适配器时）按"页面里有什么"逐级降级**：先读页面内嵌的 schema.org `JobPosting` 结构化数据——那是有公开规范的一级，零模型、零选择器、可离线测；没有就把页面当列表页，挑出像岗位详情页的链接派发出去。链接挑选是启发式且**刻意放宽**：多挑一个的代价是多取一个页面（抽不出岗位，什么也不会入库），漏挑的代价是漏岗位；真正兜住错误的是对账。只跟进**同主机**链接——跨站的多半是社交分享、母公司官网、招聘平台外链。
- **站点地图的差额不是漏抓数**。地图里会有早就招满、URL 却还留着的岗位，所以"地图里有 N 个没抓到"既有可能是我们漏了，也有可能是那些岗位已经下架——**不逐个访问就无法区分**。它产出的因此是「无法确认 + 一批待核实候选」，而不是"已确认不全：差 N 条"。反过来，覆盖了地图全集也只是**正向证据**：地图自己也可能不全，不足以单独下"已确认为全量"。把这两个方向都当成硬结论，就是拿不确定冒充确定。
- **站点地图是不可信 XML**：`DOCTYPE` / `ENTITY` 声明在**解析之前**就被拒绝。billion laughs 能把几 KB 的文档炸成几 GB，等解析器开始展开就晚了；XXE 则能读本地文件。**刻意不引入 `defusedxml`**——本功能的输入面只有"响应体大小已封顶的 XML"，一条前置检查足够，而新增依赖要钉版本、要过许可证审查，代价大于收益。
- 两张表：`official_site`（源：识别结果 + 端点 + 配方 + robots 结论 + 限速画像）与 `official_collect_run`（每次采集的账目、三态结论、逐层依据、模型用量）。后者同时是**趋势离群的基线来源**——计数不必另开一张表：运行记录里已经有 `collected`，还带着这次的状态与阻断分类，而趋势判断正需要那两项（只有有效测量才进基线）。
...
| 官网采集不复用投递适配器       | 读与写是两个契约：`SiteAdapter` 要求实现 `open_apply` / `fill_and_submit`，而采集是纯读且大多走 HTTP。合并会让每个采集适配器都实现一个永远用不到的投递方法。两者共享数据结构、各自定义行为接口，注册表并列 |
| 完整性结论只有三态、不带百分比 | 一次被 429 截断的采集同样"没报错"，只是少了一半岗位。因此翻页终止走双信号判定（内容侧才算"确认到底"，传输侧一律记为"不知道"），结论只给「已确认为全量 / 已确认不全 / 无法确认」并附逐层依据。百分比会给出虚假的精确感，且无法区分"抓全了"与"被拦住了没看到" |
| 阻断分类是一等公民           | 取回层把 429 / 403 / 验证码 / 登录墙 / **软封禁（200 但空壳页）** / 超时 / 截断都收敛成分类，因为**状态码 200 不等于拿到了内容**，而分类结果直接决定对账结论。判据宁可多判阻断（假阳性只让结论保守降级，假阴性会让报告自信地宣称已确认为全量），但 marker 只用明确整句，不用「登录」这种单词 |
| robots 判定装在传输层         | robots 按**主机**生效，而一次采集至少碰到用户给的域名与适配器实际调用的接口域名两个主机。做成传输层装饰器，适配器拿不到裸传输，就不存在"某个适配器忘了查"。读不到规则时保守视为不允许——"读不到就当作没有规则"会把站点故障期变成被加压的窗口 |
| 编排用地址队列而非游标翻页     | 接口型站点（一个端点返回全部）与列表页型站点（列表页 → 一堆详情页）是两种爬取形状。泛化 `FeedPage` 多一个 `next_targets`，换来去重/落库/限速/对账四件事**仍只有一份实现**；另写一套编排会让它们各有两份，而两份必然漂移 |
| 站点地图的差额不当漏抓数       | 地图里的 URL 可能是早就招满、链接还留着的岗位，不逐条访问分不出"漏抓"与"已下架"，所以产出的是「无法确认 + 待核实候选」。反方向同理：覆盖了地图全集只是正向证据，地图自己也可能不全。两个方向都当硬结论就是拿不确定冒充确定 |
| 存活校验的判据刻意不对称       | 把「还在招」误判成「已下架」会让对账宣称抓全了，而用户会在漏掉的岗位上投简历而不自知——这是本功能最坏的结果。所以「已下架」只认 404/410 与过期的 `validThrough` 两种结构化证据，**不认文案判据**（"职位已关闭"之类在多数站点成立，但误判方向正好是危险的那一侧）。判不出来就返回「无法判断」 |
| 官网采集后台跑、停止是协作式的 | 多页站点是几十次请求加限速等待，同步走完会把 HTTP 请求挂住且无法中途停下。采集在后台任务里跑、接口立刻返回运行 id；停止只是置一个标记，采集器在下一个检查点自己停下来——**强杀会让记录停在半成品状态**，而协作式停下时账目完整、照常给出对账结论 |
| 传输实现只有一个注入点         | 后台任务自己构造传输对象，没有调用方能注入的位置。因此 `service.default_http_factory(db)` 是全项目唯一构造传输的地方，接口路径与后台任务都走它，返回**组合好的**传输（HTTP + 渲染升级）。**换不掉的后果不是"测试难看"，而是用例会真的去请求外部站点**——在能联网的开发机上"碰巧通过"、在 CI 上挂到超时，表现为"采集一直停在采集中" |
| 趋势提醒不参与对账             | 条数下降有两个来源——站点改版导致我们少抓，或公司真的关掉了岗位——**只看条数分不出来**。把它塞进 `reconcile` 会让"公司缩减招聘"这种正常情况被报成故障，所以它是一条独立提醒，措辞把两种可能都摆出来。三个细节：基线取**中位数**（一次"只返回了一半"的采集会把平均数拽低、掩盖真实掉量）、**只有有效测量才进基线**（被阻断/停止/失败的 0 是"我们没拿到"，混进去会把基线越拉越低）、**比例与绝对差额同时超标**才算（4 条掉到 2 条也是腰斩，但那多半是正常增减） |
| 通用路径的抽取分四级、按代价排序 | 结构化数据 → 默认规则（链接文字即岗位名）→ 归纳出的配方 → 模型。前一级读得出东西就不问下一级，四级只有一处实现（`generic/listing.py`）。**默认规则放在适配器里而不是编排层**：它零成本、零猜测，与 JSON-LD 同一类；归纳出的配方与模型要读站点记忆、要用用户配的 key，属于编排层 |
| 配方用结构签名而不是 CSS 选择器 | 生成方与匹配方是同一份契约，签名集合由生成器闭合——不会出现"配方里的写法匹配器不认识"这类只在用户机器上出现的问题。相似度重定位本来就要打分匹配，用 CSS 则精确与模糊两套并存、必然漂移。另一个硬约束是不引依赖：开发机 venv 里有 `bs4`/`lxml`，但它们没写进 requirements，发布包里根本没有 |
| 配方治结构漂移、不治改名     | 站点改版时多包一层 wrapper（规则一律相对岗位链接表达）、class 增补（精确命中是子集判定）、签名里的特征掉了一部分（相似度按覆盖率算）都能救回来；class 被整个改名、标签被换掉时**如实判失效**并退回上一级——硬匹配出来的是"位置错误却看似正常"，没有任何下游信号能发现它 |
| 模型给数据、不给选择器       | 让模型直接给选择器，那串东西无从校验、错了也没有下游信号。改成：模型只回答"第几条是岗位、叫什么、在哪"（每条地址都必须在页面上真实存在），**选择器由我们反推**，再用反推的配方**重放整页**——读不出模型给的那批岗位、或读出的条数远超出一倍，就不落库 |
| 模型成本按"一次采集"封顶     | 单页上限挡不住"五十个详情页各问一次"。四道闸：内容哈希未变就跳过（**只记"问过且什么都没读出来"的**，读出了内容就不该跳过）、喂压缩后的清单、只重试一次且只在结构错时、一次采集的总上限。报告里逐页写明花了多少次调用——用户自付 key，这一项必须可见 |
| 历史公司发现接口不再是官网采集入口 | 旧的发现路由与历史表只为兼容旧客户端 / 旧数据保留；当前产品要求用户自己添加公司官网或招聘页，不再提供岗位需求搜公司流程 |
| 渲染升级做成传输层装饰器       | 与 `RobotsAwareHttp` 同一个形状：适配器拿到的仍是一个 `FeedHttp`，不知道背后发生了升级。把判断塞进适配器，等于让每个适配器各写一遍"这一页要不要渲染"，而漏写的那个就静默漏抓。组合顺序有硬要求——robots 闸门必须在**外层**，否则浏览器取回会绕过它：**换一个传输不该换掉规则** |
| 判断"要不要渲染"两边都不误伤   | 只看"文字少"会把正常的空结果（真没有岗位、404 页）也送去渲染，白开一次浏览器；只看"有前端痕迹"会把预渲染过的页面也送去渲染。所以要求**两条同时成立**。而升级失败必须留痕——空壳会被上层读成"这里没有岗位"，那正是漏抓被伪装成正常结果的形态 |
| 结论重算靠重放存储的输入       | 存活校验之后要重算结论，而重算必须用原来的账目。把对账输入随运行记录存下来、原样重放同一个 `reconcile`，而不是从记录里反推（"终止状态是 limit，所以 reached_page_limit 应该是 True"）——反推既脆弱又会悄悄偏离原值，而偏差会让同一份账目算出两个不同结论 |
