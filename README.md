...
| 模块               | 说明                                                                                                             |
| ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| 📋 岗位管理        | 手动填写、粘贴跨行业招聘信息或读取公开岗位详情链接生成草稿；保存时提取技能标签                                   |
...
自定义模型需要兼容 OpenAI Chat Completions 请求和响应格式：填写 `Base URL` + `模型名称`，云服务再按要求填写 `API Key`，测试成功后保存即可。远程地址必须使用 HTTPS，HTTP 只允许本机回环地址。配置保存在本地数据库中，浏览器只会收到脱敏占位符；测试连接和模型调用时，后端才会把已配置的密钥作为鉴权信息发送到你主动配置的服务商。修改已保存配置的 `Base URL` 时，需要重新填写 API Key。常用配置可命名保存到“配置记录”并随时切换，不需要数据库迁移。
...
│   │   ├── services/       # 业务逻辑：简历生成、岗位页面/文本/JD 解析、导出
