...
当前版本：`0.7.0` · [Release 页面](https://github.com/Magicapple-Coder/ResumeForge/releases)
...
| 模块               | 说明                                                                                                             |
| ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| 📋 岗位管理        | 手动填写或粘贴跨行业招聘信息生成草稿；保存时提取技能标签，并在备注里自动标注来源（粘贴文本 / 图片 / 文档识别）         |
...
| 🤖 AI 定制简历     | 按 JD 筛选事实并流式生成，可选择美化拓展强度、最大篇幅（1-3 页 A4）、字号与模板（经典 / 现代 / 精简），还能补充自定义要求 |
...
| 💬 AI 求职助手     | 使用设置中的模型进行流式问答，保存会话历史；可显式关联岗位、简历、资料，上传文本或图片并按需联网搜索；首次进入有内置引导对话 |
| 🧩 助手技能        | 在「技能工作台」新建、导入和编辑技能（提示词 + 知识文件），启用后约束助手作答风格；助手页可以直接开关，知识由助手按需读取 |
| 🗒 会话管理        | 每条会话支持重命名、置顶、收藏、复制分享链接、在新对话中继续、移动到分组、归档与删除；消息带发送时间，也可调思考强度      |
...
| 🔄 检查与更新      | 设置页可检查新版本；Windows 下运行 `update.cmd` 一键更新程序文件，`data/` 与 `.env` 不受影响                     |
| 🔑 你的 Key 你做主 | 内置 15 个常见服务预设（DeepSeek/豆包/Kimi/通义/智谱/MiniMax/硅基流动/OpenRouter/OpenAI/Gemini/Grok/Groq/Mistral/Ollama/LM Studio），可获取当前账号可用模型并做高级参数调整 |
...
**没有内置预设的模型如何配置？**
在设置页点击「编辑设置」，在「快速预设」里选择「自定义模型（OpenAI 兼容）」（保留当前内容）或「纯手动配置（不套用任何预设）」（清空 Base URL 与模型名，从空白开始填），然后填写服务商的 `Base URL` 和模型名称；云服务按要求填写 API Key，无需鉴权的本地 Ollama 或其他兼容服务可以留空。`Base URL` 可填写服务根地址（例如以 `/v1` 结尾）或完整的 `/chat/completions` 地址；远程服务必须使用 HTTPS，本机回环地址可使用 HTTP，地址不能包含账号、密码、查询参数或片段。当前仅直接支持 OpenAI Chat Completions 兼容协议，原生 Claude、Gemini 等接口需要兼容网关。先测试连接并保存，再按需保存为配置记录。
