...
**联网搜索会自动抓取招聘网站吗？**
不会。开启联网搜索后，查询会发送给固定的 Bing RSS 搜索接口，最多读取少量公开结果的标题、URL 和摘要，不跟随结果链接，也不抓取网页正文。助手被要求在寻找招聘信息时优先采用公司、学校、医院、政府或其他用人单位的官方页面，并把第三方页面仅作为线索；但搜索摘要可能不完整、过期或仍包含第三方结果，投递前必须打开来源并自行核验是否为官方页面。
