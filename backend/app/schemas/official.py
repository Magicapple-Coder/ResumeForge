...
class OfficialDiscoverQuery(BaseModel):
    """按岗位需求找公司。``city`` 可空——不少岗位名本身就带地域，或用户并不限定城市。"""

    keywords: str = Field(default="", max_length=200)
    city: str = Field(default="", max_length=64)


class OfficialCandidateOut(BaseModel):
    """一条候选公司线索。

    ``company`` 是**建议名**（从标题或域名猜的），界面必须允许用户改——猜错不该由用户承担。
    ``target_field`` 告诉前端这个地址该填进新增公司表单的哪个字段，由服务层定好：让前端自己
    按地址猜一遍，就会出现"清单里认成招聘页、提交后却被当成首页"这类两边判据不一致的问题。
    """
    ...
    target_field: str = "careers_url"


class OfficialDiscoveryOut(BaseModel):
    """一次发现的结果。

    ``detail`` 里带着"这是线索、不是完整名单"的说明，界面**必须原样显示**：用户把线索当名单，
    就会漏掉一大批公司而不自知。
    """

    history_id: int | None = None
    candidates: list[OfficialCandidateOut] = Field(default_factory=list)
    # 实际发出去的查询词。要显示——用户才能判断我们是怎么找的、要不要换个词再找。
    queries: list[str] = Field(default_factory=list)
    detail: str = ""


class OfficialDiscoveryHistoryOut(BaseModel):
    """一次历史搜索及其当时的候选快照。"""
