...
**2026-09-26 移除了整个「官网采集」功能**：服务层（``services/sites/official/``）、接口、
schema、助手工具、前端页面与组件、以及它们的测试都已删除。**但本模块与下面三个模型类
故意保留**，因为它不是"功能的代码"，是"数据库结构的声明"：

- ``data_backup`` 会**拒收**含"当前版本不认识的数据表"的备份，而"当前版本认识哪些表"是从
  ``Base.metadata`` 推导的（``database_migrations.application_tables()``）；
- 只删模型不删表 → 用户已有的备份在导入时被判成"来自更新版本"而**全部拒收**。

所以表与模型**必须同生同死**。现在两者都保留，等下一次本来就要加迁移时一并删掉——
清单与做法见 ``AGENTS.md`` 的「待办」节。在那之前这三张表不写不读，只是历史数据。

## 下面这些常量还有用吗

本模块同时是**采集记录里那些字段的取值域**（阻断分类、对账三态、运行状态），它们随
``official_collect_run`` 的列一起留在历史数据里。删掉它们不会让旧数据显示不出来
（那些列读出来是字符串），但会让"这些值是干什么的"重新变成口口相传——所以一并留着。

本模块**不含站点知识**：哪家公司在用哪套招聘系统属于适配器层，而那一层已经随功能删除了。
"""
