...
def test_parse_full_job_posting_extracts_core_fields_and_sections():
    ...
    assert result.description.splitlines()[:3] == ["正式", "研发 - 客户端", "职位 ID：A134186"]
    assert "团队介绍：剪映研发团队" in result.description
...
def test_parse_empty_description_warns_even_when_preamble_metadata_is_preserved():
    ...
    assert result.description.splitlines() == ["正式", "研发 - 客户端", "职位 ID：A100"]
